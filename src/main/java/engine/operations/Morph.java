...
public class Morph {
    private static final Log log = LogFactory.getLog(Morph.class);
    ...
    public ArrayList<Integer> findLemmaIndexInText(String text, String lemma) {
        ...
        for(String string : list) {
            ...
            try {
                lemmas = morphology.getNormalForms(string.toLowerCase(Locale.ROOT));

            } catch (Exception e) {
                log.debug("Ошибка морфологического анализа. Пропущенные символы: " + string);
            }
            for(String string1 : lemmas) {
                if (string1.equals(lemma)){
