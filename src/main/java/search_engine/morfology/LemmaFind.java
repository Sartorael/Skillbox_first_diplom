    package search_engine.morfology;
    ...
    public class LemmaFind {

        private final static Log log = LogFactory.getLog(LemmaFind.class);
        ...
        public static List<Search> getSnippetList(String text, Set<String> lemmas) {

            List<Search> searchList = new ArrayList<>();
            String textCleaned = removeHTMLTags(text);
            String[] fragments = textCleaned.split("<\n>");
            for (String fragment : fragments) {
                Search search = new Search(fragment);
                Matcher matcher = getMatcherRussianWord(fragment);
                while (matcher.find()) {
                    String baseWord = getNormalForm(matcher.group());
                    if (lemmas.contains(baseWord)) {
                        search.getQueryWordsIndexes().put(matcher.start(), matcher.group());
                    }
                }
                if (!search.getQueryWordsIndexes().isEmpty()) {
